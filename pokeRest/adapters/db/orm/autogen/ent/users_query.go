...
type UsersQuery struct {
	...
	// eager-loading edges.
	withUserTrainedPokemon       *TrainedPokemonsQuery
	withUserTrainedPokemonDetail *TrainedPokemonDetailsQuery
...
// QueryUserTrainedPokemon chains the current query on the "user_trained_pokemon" edge.
func (uq *UsersQuery) QueryUserTrainedPokemon() *TrainedPokemonsQuery {
	query := &TrainedPokemonsQuery{config: uq.config}
...
// QueryUserTrainedPokemonDetail chains the current query on the "user_trained_pokemon_detail" edge.
func (uq *UsersQuery) QueryUserTrainedPokemonDetail() *TrainedPokemonDetailsQuery {
	query := &TrainedPokemonDetailsQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UsersQuery) Clone() *UsersQuery {
	...
	return &UsersQuery{
		...
		withUserTrainedPokemon:       uq.withUserTrainedPokemon.Clone(),
		withUserTrainedPokemonDetail: uq.withUserTrainedPokemonDetail.Clone(),
...
// WithUserTrainedPokemon tells the query-builder to eager-load the nodes that are connected to
// the "user_trained_pokemon" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UsersQuery) WithUserTrainedPokemon(opts ...func(*TrainedPokemonsQuery)) *UsersQuery {
	query := &TrainedPokemonsQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserTrainedPokemon = query
	return uq
}

// WithUserTrainedPokemonDetail tells the query-builder to eager-load the nodes that are connected to
// the "user_trained_pokemon_detail" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UsersQuery) WithUserTrainedPokemonDetail(opts ...func(*TrainedPokemonDetailsQuery)) *UsersQuery {
	query := &TrainedPokemonDetailsQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserTrainedPokemonDetail = query
	return uq
}
...
func (uq *UsersQuery) sqlAll(ctx context.Context) ([]*Users, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [2]bool{
			uq.withUserTrainedPokemon != nil,
			uq.withUserTrainedPokemonDetail != nil,
		}
	)
	...
	if query := uq.withUserTrainedPokemon; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Users)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserTrainedPokemon = []*TrainedPokemons{}
		}
		query.Where(predicate.TrainedPokemons(func(s *sql.Selector) {
			s.Where(sql.InValues(users.UserTrainedPokemonColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.UserTrainedPokemon = append(node.Edges.UserTrainedPokemon, n)
		}
	}

	if query := uq.withUserTrainedPokemonDetail; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Users)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserTrainedPokemonDetail = []*TrainedPokemonDetails{}
		}
		query.Where(predicate.TrainedPokemonDetails(func(s *sql.Selector) {
			s.Where(sql.InValues(users.UserTrainedPokemonDetailColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.UserTrainedPokemonDetail = append(node.Edges.UserTrainedPokemonDetail, n)
		}
	}
