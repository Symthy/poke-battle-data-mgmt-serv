...
func (tx *Tx) init() {
	tx.Abilities = NewAbilitiesClient(tx.config)
	tx.BattleOpponentParty = NewBattleOpponentPartyClient(tx.config)
	tx.BattleRecords = NewBattleRecordsClient(tx.config)
	tx.Forms = NewFormsClient(tx.config)
	tx.HeldItems = NewHeldItemsClient(tx.config)
	tx.Moves = NewMovesClient(tx.config)
	tx.Party = NewPartyClient(tx.config)
	tx.PartyResultRecord = NewPartyResultRecordClient(tx.config)
	tx.Pokemons = NewPokemonsClient(tx.config)
	tx.Tags = NewTagsClient(tx.config)
	tx.TrainedPokemonDetails = NewTrainedPokemonDetailsClient(tx.config)
	tx.TrainedPokemons = NewTrainedPokemonsClient(tx.config)
	tx.TypeCompatibility = NewTypeCompatibilityClient(tx.config)
...
// applies a query, for example: Abilities.QueryXXX(), the query will be executed
