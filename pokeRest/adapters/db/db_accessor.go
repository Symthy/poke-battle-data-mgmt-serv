...
// Todo: wrap and adapter of db
type GormDbAccessor struct {
	db *gorm.DB
}

func NewGormDbAccessor() *GormDbAccessor {
	da := &GormDbAccessor{}
	da.initialize()
	return da
}

func (da *GormDbAccessor) initialize() {
...
func (da GormDbAccessor) GetDb() *gorm.DB {
	return da.db
}

func (da GormDbAccessor) Paginate(page int, pageSize int) func(db *gorm.DB) *gorm.DB {
